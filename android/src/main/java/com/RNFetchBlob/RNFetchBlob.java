...
public class RNFetchBlob extends ReactContextBaseJavaModule {
     ...
     */
    @ReactMethod
    public void readStreamTailF(final String path, final String encoding, final int bufferSize, final int tick, final String streamId) {
        final ReactApplicationContext ctx = this.getReactApplicationContext();
        fsThreadPool.execute(new Runnable() {
            @Override
            public void run() {
                RNFetchBlobFS fs = new RNFetchBlobFS(ctx);
                fs.readStreamTailF(path, encoding, bufferSize, tick, streamId);
            }
        });
    }
